...
save_mapsswe_results <- function(df, is_control){
  ...
  for (version in standardization_version) {
    ...
    result <- pairwise_mapsswe_test(asr_df, numeric_var = "WER", group_var = "ASR",all_groups=  c("Google Chirp","Microsoft Azure","OpenAI Whisper","Rev AI","AssemblyAI","Amazon AWS"))
...
mapsswe_df <-save_mapsswe_results(control_merged_data,is_control = TRUE)

mapsswe_df<-save_mapsswe_results(aphasia_merged_data,is_control = FALSE)



mapsswe_df %>%
  mutate( star = ifelse(P_value_Two_Tail<0.05,"*",
                        ifelse(P_value_Two_Tail<0.01,"**",
                               ifelse(P_value_Two_Tail<0.001,"***","n.s.")))) %>% 
  mutate( Stat = paste0(round(Mean_Difference,digits = 2),"& ",round(W_Statistic,digits = 2),star),
         ) %>% 
  dplyr::select(Comparison,Version,Stat) %>% 
  pivot_wider(names_from = Version,values_from =Stat) %>% kable(format = "latex")
```
...
clustered_se <- sqrt(diag(vcov_matrix))
...
## Regression-WER-Demographic-only
```{r}

lm_v3_v3_matched2<- lm(WER~ is_female + is_aphasia + age +I(age^2)+ edu_levels  +is_OTH+ is_AA +ASR,matched_data)



# Clustered regression for unmatched data
vcov_matrix2 <- vcovHC(lm_v3_v3_matched2)
clustered_se2 <- sqrt(diag(vcov_matrix2))
cl2 <- coeftest(lm_v3_v3_matched2, cluster = ~Participant_ID,vcov. = vcov_matrix2)



stargazer(lm_v3_v3_matched2,lm_v3_v3_matched,single.row = TRUE,
          dep.var.labels = "WER",
          se=list(clustered_se2,clustered_se),
          covariate.labels = c("Gender (Female)",
                               "Aphasia",
                               "Age",
                               "Age 2",
...
# Table S12  Mixed effects model
```{r}
lmer_v3_v3_unmatched1 <- lmer(WER~ is_female + is_aphasia + age + I(age^2)+edu_levels  +is_OTH+ is_AA + Mean_Background_Noise +word_count_RFFRR+total_audio_duration+non_vocal_percentage+ASR+(1|Participant_ID)+(1|segment_name),all_data)
lmer_v3_v3_unmatched2 <- lmer(WER~ aphasia_TypeFluency +is_female +age +I(age^2)+ edu_levels  +is_OTH+ is_AA + Mean_Background_Noise +word_count_RFFRR+total_audio_duration+non_vocal_percentage+ASR+(1|Participant_ID)+(1|segment_name),all_data)

this_df <- all_data %>% filter(aphasiaTypeBoston %in% c("control","Anomic","Conduction","Wernicke","Broca","Global") ) 

this_df$aphasiaTypeBoston <-factor(this_df$aphasiaTypeBoston,levels = c("control","Anomic","Conduction","Wernicke","Broca","Global"))
lmer_v3_v3_unmatched3 <- lmer(WER~ aphasiaTypeBoston +is_female +age +I(age^2)+ edu_levels  +is_OTH+ is_AA + Mean_Background_Noise +word_count_RFFRR+total_audio_duration+non_vocal_percentage+ASR+(1|Participant_ID)+(1|segment_name),this_df)

tab_model(lmer_v3_v3_unmatched1,lmer_v3_v3_unmatched2,lmer_v3_v3_unmatched3,p.style = "star",show.se =TRUE,show.ci = FALSE,collapse.se = TRUE)
```
